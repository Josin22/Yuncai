...
- (void)updateGoodModel:(YBLGoodModel *)model{
    
    [self.goodImageView js_scale_setImageWithURL:[NSURL URLWithString:model.avatar_url] placeholderImage:smallImagePlaceholder];
    self.goodTitleLabel.text = [NSString stringWithFormat:@"%@",model.title];
    NSString *priceString = [NSString stringWithFormat:@"%.2f",model.price.doubleValue];
    self.goodPriceLabel.attributedText = [NSString price:priceString color:YBLThemeColor font:18];
}
