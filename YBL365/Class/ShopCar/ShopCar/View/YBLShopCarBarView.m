...
@implementation YBLShopCarBarView
...
- (void)createSubViews {
    ...
    self.checkButton.frame = CGRectMake(0, 0, 72, kBottomBarHeight);
    [self addSubview:self.checkButton];
    
    self.normalView = [[UIView alloc] initWithFrame:CGRectMake(self.checkButton.right, 0, self.width-self.checkButton.right, self.height)];
    ...
    [totalLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.height.equalTo(@(self.normalView.height));
//        make.centerY.equalTo(@(self.centerY));
    }];
    
    self.realPriceLabel = [[UILabel alloc] init];
    self.realPriceLabel.attributedText = [NSString price:@"00.00" color:YBLThemeColor font:16];
    ...
    */
    ...
    [self.buyButton setTitleColor:YBLColor(150, 150, 150, 1) forState:UIControlStateDisabled];
    [self.buyButton setBackgroundColor:YBLColor(210, 210, 210, 1) forState:UIControlStateDisabled];
    [self.buyButton setBackgroundColor:YBL_RED forState:UIControlStateNormal];
...
- (void)setGoodNumber:(NSInteger)goodNumber {
    NSString *numstr = [NSString stringWithFormat:@"去结算(%ld)",(long)goodNumber];
    ...
    self.buyButton.enabled = goodNumber != 0?YES:NO;
}

- (void)updateWithType:(BarType)carType{
    self.checkButton.selected = NO;
    if (carType == BarTypeNormal) {
        self.normalView.hidden = NO;
        self.editView.hidden = YES;
    }else if (carType == BarTypeEdit){
